...
public class M2App {
	...
	public static void main(String[] args) {
		...
		String nombre, apellido, puesto, num;
		int sueldo, numPagas;
		boolean bonus, paga;
		...
		puesto = JOptionPane.showInputDialog("Cual es tu puesto? (Boss/Manager/Senior/Mid/Junior/Volunteer)")
				.toLowerCase();
		bonus = Boolean.parseBoolean(JOptionPane.showInputDialog("Tienes bonus? (true/false)"));

		// En funcion de su puesto creamos un objeto u otro
		if (puesto.equals("boss")) {
			Boss emp = new Boss(nombre, apellido, sueldo, bonus);
			// Con esto comprovamos que su salario sea el correspondiente
			if (emp.validarSueldo(emp.getSalarioMin())) { // Si lo es calculamos sus salarios neto/bruto anual/mensual
			...
			Manager emp2 = new Manager(nombre, apellido, sueldo, bonus);
			...
			Senior emp3 = new Senior(nombre, apellido, sueldo, bonus);
			...
			Mid emp4 = new Mid(nombre, apellido, sueldo, bonus);
			...
			Junior emp5 = new Junior(nombre, apellido, sueldo, bonus);
			...
			if (sueldo == 0) {
				...
				Volunteer emp5 = new Volunteer(nombre, apellido, sueldo, paga, bonus);
